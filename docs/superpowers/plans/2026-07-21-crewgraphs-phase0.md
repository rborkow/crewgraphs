...
- [ ] R2 bucket `crewgraphs-raw` + API token
- [ ] GitHub repo + secrets (NEON_DATABASE_URL, R2_ACCOUNT_ID, R2_ACCESS_KEY_ID, R2_SECRET_ACCESS_KEY)
- [ ] Hyperdrive config → Neon; KV namespace; paste IDs into `apps/web/wrangler.jsonc`
- [ ] `wrangler login`; first deploy; bind crewgraphs.com custom domain; CF Access policy on `/admin/*`
- (Full detail in `SETUP.md` once scaffold lands)
...
- [ ] `004_staging_ops.sql` — staging tables, ops.ingest_run/quarantine/publish_snapshot
- [ ] `005_read_models.sql` — read.* tables per spec contract + published_snapshot pointer + admin_v views
- [ ] Verify: `dbmate up` clean on fresh Postgres; `dbmate rollback` × 5 clean; ci-db green

### Task 1.2: Contracts package hardening `[lead defines, luna implements]`
- [ ] Finalize SourceRef v1 + profile-payload zod schemas from spike learnings; export JSON Schemas; kysely-codegen wired with drift check in ci-web
- [ ] Python side: schema-validation test harness in `pipeline/tests/` loading `packages/contracts/schemas/*.json`

### Task 1.3: Fixture cohort `[luna, lead reviews stories]`
- [ ] `db/fixtures/fixtures.sql` — ~12 fake orgs covering every state: full 5-yr 990, 990-EZ-only, 990-N-only, missing middle year, amended filing, under_review value, partial metric, single-filing newcomer, legal≠display name, renamed org (slug history), peer-cohort overlaps, zero-revenue year
- [ ] Fixture payloads validate against contracts schemas in CI

### Task 1.4: Pipeline run harness `[terra]`
- [ ] `ops.ingest_run` lifecycle wrapper, R2 client (write-once enforcement), config/secrets loading, quarantine writer, Actions job-summary emitter
- [ ] Unit tests with a fake R2 (local dir) + ephemeral Postgres

**Phase 1 exit (gate):** one PR containing migrations + contracts + regenerated types + fixtures, co-reviewed against both the web and data workstream plans; ci-web/ci-db/ci-ingest all green.
